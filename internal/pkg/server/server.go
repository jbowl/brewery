...
func serverOpts() ([]grpc.ServerOption, error) {
	opts := []grpc.ServerOption{}
	var creds credentials.TransportCredentials
	var err error
	switch os.Getenv("INSECURE") {
	case "TRUE":
		opts = []grpc.ServerOption{}
	case "LOCAL":
		creds, err = credentials.NewServerTLSFromFile("./jbowl.pem", "./jbowl.key")
		opts = []grpc.ServerOption{grpc.Creds(creds)}
	case "": // default use prod cert
		creds, err = credentials.NewServerTLSFromFile("./jbowl.cert", "./jbowl.key")
		opts = []grpc.ServerOption{grpc.Creds(creds)}
...
// change holdapi.Filter to hold only query, so mabey only use query
//   research generics for containers to overload comparison
